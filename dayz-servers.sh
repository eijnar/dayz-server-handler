...
SERVER_CFG_TEMPLATE="${SCRIPT_DIR}/serverDZ.cfg.tpl"
...
BASE
  ...
  local stub_map
  for stub_map in chernarus sakhal; do
    ensure_map_mod_stub "${stub_map}"
  done
}

ensure_server_cfg_template(){
  if [[ ! -f "${SERVER_CFG_TEMPLATE}" ]]; then
    cat > "${SERVER_CFG_TEMPLATE}" <<'TPL'
...
steamMasterServerPort = {{STEAM_MASTER_PORT}};
steamStatistics = 1;
...
TPL
    echo "Created ${SERVER_CFG_TEMPLATE}. Adjust defaults there if needed."
  fi
}
...
write_server_cfg(){
  ...
  [[ -f "${cfg}" ]] && return 0
  [[ -f "${SERVER_CFG_TEMPLATE}" ]] || die "Server config template not found at ${SERVER_CFG_TEMPLATE}"

  local hostname="My DayZ Server (${map})"
  local instance_name
  instance_name="$(basename "${inst_dir}")"
  local instance_id="${game_port}"

  local tmp
  tmp="$(mktemp "${inst_dir}/serverDZ.cfg.XXXXXX")"

  sed \
    -e "s|{{HOSTNAME}}|$(escape_sed "${hostname}")|g" \
    -e "s|{{MISSION_TEMPLATE}}|$(escape_sed "${mission_template}")|g" \
    -e "s|{{DIFFICULTY}}|$(escape_sed "${difficulty}")|g" \
    -e "s|{{MAP}}|$(escape_sed "${map}")|g" \
    -e "s|{{INSTANCE_NAME}}|$(escape_sed "${instance_name}")|g" \
    -e "s|{{INSTANCE_ID}}|${instance_id}|g" \
    -e "s|{{GAME_PORT}}|${game_port}|g" \
    -e "s|{{STEAM_QUERY_PORT}}|${query_port}|g" \
    -e "s|{{STEAM_MASTER_PORT}}|${master_port}|g" \
    "${SERVER_CFG_TEMPLATE}" > "${tmp}"
